 ...
 */
class read_latest_logged_students extends \external_api {
     ...
     */
    public static function latest_logged_students_read_parameters() {
    ...
    /**
     * Returns the students who logged in latest to Moodle.
     *
     * @param text name Required param based on which the user is looked up in database
     * @return An array of students and warnings.
     */
    public static function latest_logged_students_read($limit) {
        global $USER, $DB, $PAGE;
        $studentsarray = [];
        $warnings = [];
        $courses = [];
        $params = self::validate_parameters(
            self::latest_logged_students_read_parameters(),
            array(
                'limitnumber' => $limit
            )
        );
     ...
     */
    private static function get_latest_logged_students_structure() {
    ...
    /**
     * Describes the return value for get_latest_logged_students
     *
     * @return external_single_structure
     */
    public static function latest_logged_students_read_returns() {
        return new \external_single_structure(
            array(
                'students' => new \external_multiple_structure(self::get_latest_logged_students_structure(), 'students logged in latest details', VALUE_DEFAULT, []),
